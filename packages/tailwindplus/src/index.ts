...
import { collectExtends, expandExtendCandidates, mergeExtendSelectors } from './extend'
...
async function parseCss(
  ...
  let extendGraph = collectExtends(ast, designSystem)
  if (extendGraph.extendMap.size > 0 || extendGraph.conditionalDeps.size > 0) {
    features |= Features.AtExtend
  }
...
export async function compileAst(
  ...
  let { designSystem, ast, sources, root, utilitiesNode, features, inlineCandidates, extendGraph } =
  ...
  for (let candidate of extendGraph.forcedCandidates) {
    if (!designSystem.invalidCandidates.has(candidate)) {
      allValidCandidates.add(candidate)
      defaultDidChange = true
    }
  ...
  if (expandExtendCandidates(allValidCandidates, extendGraph, designSystem)) {
    defaultDidChange = true
  }
  ...
  return {
    ...
    build(newRawCandidates: string[]) {
      ...
      if (!utilitiesNode) {
        if (!compiled) {
          mergeExtendSelectors(ast, extendGraph, allValidCandidates, designSystem)
