...
import type { AstNode } from '../../tailwindplus/src/ast';
import { getModuleDependencies } from './get-module-dependencies';
import { rewriteUrls } from './urls';

declare global {
  var __tw_load: ((path: string) => Promise<any>) | undefined;
  var __tw_resolve: ((id: string, base: string) => string | undefined) | undefined;
}
...
async function ensureSourceDetectionRootExists(compiler: { root: Awaited<ReturnType<typeof compile>>['root'] }) {
...
export async function compileAst(ast: AstNode[], options: CompileOptions): Promise<{
...
export async function compile(css: string, options: CompileOptions): Promise<{
  sources: { base: string; pattern: string; negated: boolean }[];
...
export async function __unstable__loadDesignSystem(css: string, { base }: { base: string }): Promise<any> {
...
async function loadStylesheet(id: string, base: string, onDependency: (path: string) => void, cssResolver?: Resolver) {
...
async function importModule(path: string): Promise<any> {
  if (typeof globalThis.__tw_load === 'function') {
    const module = await globalThis.__tw_load(path);
    if (module) {
      return module;
    }
  }
...
async function resolveCssId(
  ...
  if (typeof globalThis.__tw_resolve === 'function') {
    const resolved = globalThis.__tw_resolve(id, base);
    if (resolved) {
...
async function resolveJsId(id: string, base: string, customJsResolver?: Resolver): Promise<string | false | undefined> {
  if (typeof globalThis.__tw_resolve === 'function') {
    const resolved = globalThis.__tw_resolve(id, base);
    if (resolved) {
      return Promise.resolve(resolved);
